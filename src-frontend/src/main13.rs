...
async fn fetch_github_repos(username: String) -> Result<Vec<GithubRepo>, String> {
    if username.is_empty() { return Ok(Vec::new()); }
    let url = format!("https://api.github.com/users/{}/repos?sort=updated&per_page=5", username);
    let response = Request::get(&url).send().await.map_err(|e| format!("Network error: {e}"))?;
    if !response.ok() { return Err(format!("GitHub API Status: {}", response.status())); }
    response.json::<Vec<GithubRepo>>().await.map_err(|e| format!("Parse error: {e}"))
}

// Proxied via backend to avoid CORS - Robust Error Handling
async fn fetch_aur_packages(maintainer: String) -> Result<Vec<AurPackage>, String> {
    if maintainer.is_empty() { return Ok(Vec::new()); }
    let url = format!("/api/aur/maintainer?arg={}", maintainer);
    
    let response = Request::get(&url).send().await.map_err(|e| format!("Proxy network failure: {e}"))?;
    
    if !response.ok() {
        let err_text = response.text().await.unwrap_or_else(|_| "Unknown server error".to_string());
        return Err(format!("Backend Error ({}): {}", response.status(), err_text));
    }
    
    response.json::<Vec<AurPackage>>().await.map_err(|e| format!("JSON Parse Error: {e}"))
}

// Proxied via backend to avoid CORS - Robust Error Handling
async fn search_aur_packages(query: String) -> Result<Vec<AurPackage>, String> {
    if query.len() < 3 { return Ok(Vec::new()); }
    let url = format!("/api/aur/search?arg={}", query);
    
    let response = Request::get(&url).send().await.map_err(|e| format!("Proxy network failure: {e}"))?;
    
    // If backend returns an error code, extract the body text so we can see the real error
    if !response.ok() {
        let err_text = response.text().await.unwrap_or_else(|_| "Unknown server error".to_string());
        return Err(format!("Backend Error ({}): {}", response.status(), err_text));
    }
    
    // Only attempt JSON parsing if we got a successful response
    response.json::<Vec<AurPackage>>().await.map_err(|e| format!("JSON Parse Error: {e}"))
}
...
async fn compile_remote(code: String, lang: String) -> Result<String, String> {
    ...
    if !response.ok() {
        ...
        return Err(format!("Backend rejected request (Status {}): {}", response.status(), error_body));
    }
