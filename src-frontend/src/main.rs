                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-[#161925] border border-[#2d204d] p-5 rounded shadow-md">
                                    <h3 class="text-md font-mono font-bold text-white mb-4 flex items-center gap-2">
                                        <span class="text-orange-400">"📊"</span> "AUR Deployment Metrics"
                                    </h3>
                                    {move || match aur_packages_resource.get().map(|w| (*w).clone()) {
                                        Some(Ok(pkgs)) => {
                                            let total_votes: u32 = pkgs.iter().map(|p| p.num_votes).sum();
                                            view! {
                                                <div class="grid grid-cols-2 gap-4">
                                                    <div class="bg-[#0d0e15] p-3 rounded border border-[#2d204d]">
                                                        <div class="text-xs text-purple-400">"Total Packages"</div>
                                                        <div class="text-2xl font-bold text-[#00ffcc]"> {pkgs.len()} </div>
                                                    </div>
                                                    <div class="bg-[#0d0e15] p-3 rounded border border-[#2d204d]">
                                                        <div class="text-xs text-purple-400">"Total Votes"</div>
                                                        <div class="text-2xl font-bold text-[#ff007f]"> {total_votes} </div>
                                                    </div>
                                                </div>
                                            }.into_any()
                                        },
                                        _ => view! { <div class="text-purple-400 text-xs">"Loading metrics..."</div> }.into_any()
                                    }}
                                </div>

                                <div class="bg-[#161925] border border-[#2d204d] p-5 rounded shadow-md flex flex-col justify-center">
                                    <h3 class="text-md font-mono font-bold text-white mb-2 flex items-center gap-2">
                                        <span class="text-blue-400">"🔗"</span> "System Linkage"
                                    </h3>
                                    <p class="text-xs text-purple-300 mb-4">"Manage deployment pipelines and codebase integrity via GitHub upstream nodes."</p>
                                    <a href="https://github.com/darkstardevx/subgrid" target="_blank" class="w-full bg-[#1f2335] hover:bg-[#2d204d] border border-[#44365d] text-center py-2 rounded text-xs font-mono font-bold text-[#00ffcc] transition-all">
